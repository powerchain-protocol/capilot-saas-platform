  ...
  return (
    <main className="min-h-screen bg-[var(--canvas)]">
      <header className="pc-shell flex min-h-20 items-center justify-between gap-4 py-3">
        <Logo />
        <Link className="text-right text-xs font-semibold text-[var(--green)] hover:underline sm:text-sm" href="/sign-in">Already have an account? Sign in</Link>
      </header>
      <section className="pc-shell pb-16 pt-6 sm:pt-10">
        <div className="mx-auto max-w-3xl text-center">
          <Link href="/pricing" className="mb-6 inline-flex items-center gap-1 text-[10px] font-bold text-[#6D7771] hover:text-[var(--green)]"><ArrowLeft className="size-3" />Back to pricing</Link>
          <p className="pc-kicker">Get started</p>
          <h1 className="pc-title mt-4 text-4xl sm:text-5xl">Create your PowerChain workspace.</h1>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[var(--muted)]">Set up your identity, organization and authenticated workspace. Your initial operational data is provisioned automatically so you can explore the full product flow.</p>
        </div>
        ...
        <div className="mx-auto mt-6 flex max-w-2xl flex-wrap justify-center gap-x-5 gap-y-2 text-[10px] font-semibold text-[#727C76]">
          {["Secure authentication", "Signed session", "Workspace-scoped access", "Policy controlled"].map((item) => <span key={item} className="flex items-center gap-1.5"><ShieldCheck className="size-3 text-[var(--success)]" />{item}</span>)}
        </div>
      </section>
    </main>
  );
