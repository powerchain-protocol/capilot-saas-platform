...
export function InstallInstructions({platform}:{platform:Platform}){return <ol className="mt-6 space-y-3">{steps[platform].map((s,i)=><li key={s} className="flex gap-3 rounded-xl border border-[var(--border)] bg-white p-4"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-[#EAF3ED] text-xs font-bold text-[var(--success)]">{i+1}</span><div><b className="text-sm">{s}</b>{i===steps[platform].length-1&&<p className="mt-1 text-[10px] text-[var(--muted-2)]">Keep signing and execution controls explicit. PowerChain never asks you to expose private keys.</p>}</div></li>)}</ol>}
