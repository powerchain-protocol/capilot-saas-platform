...
export function DashboardPreview() {
  return (
    <section id="dashboard" className="bg-[var(--canvas)] py-24">
      <div className="pc-shell">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-base leading-7 text-[var(--muted)]">Portfolio-level intelligence with evidence freshness, asset health, treasury context, and actionable exceptions.</p>
        </div>

        <div className="pc-card mt-12 overflow-hidden">
          <div className="grid min-h-[610px] lg:grid-cols-[190px_1fr]">
            <aside className="hidden border-r border-[var(--border)] bg-[#FBFCFB] p-4 lg:block">
              <Logo size="compact" priority />
              <div className="mt-7 space-y-1">
                {[["Overview",LayoutDashboard],["Assets",Sun],["Energy",Wind],["Treasury",WalletCards],["Reports",BarChart3]].map(([x,I])=><div key={String(x)} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold ${x==="Overview"?"bg-[#EDF3EF] text-[#17613F]":"text-[var(--muted)]"}`}><I className="size-4"/>{x}</div>)}
              </div>
            </aside>
            ...
            <div className="p-4 sm:p-6">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="rounded-full bg-[#EFF7F1] px-3 py-1 text-[10px] font-bold text-[var(--success)]">● Production · 12s ago</span>
              </div>

              <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
                {metrics.map((item)=><div key={item.label} className="rounded-2xl border border-[var(--border)] bg-white p-4"><div className="flex items-center gap-2 text-[var(--green)]"><span className="grid size-5 place-items-center">{item.icon}</span><span className="text-[10px] font-semibold text-[#6A756F]">{item.label}</span></div><b className="mt-3 block text-xl tracking-[-.03em]">{item.value}</b><span className="mt-1 block text-[10px] text-[var(--success)]">{item.meta}</span></div>)}
              </div>

              <div className="mt-4 grid gap-4 xl:grid-cols-[1.55fr_.85fr]">
                <div className="rounded-2xl border border-[var(--border)] bg-white p-5">
                  <div className="flex justify-between"><div><p className="text-xs font-semibold">Energy Generation</p><b className="mt-1 block text-xl">24,812 MWh</b></div><span className="text-[10px] text-[var(--success)]">▲ 12.4%</span></div>
...
function CheckIcon(){return <span className="grid size-7 shrink-0 place-items-center rounded-full bg-[#EDF6F0] text-[var(--success)]">✓</span>}
