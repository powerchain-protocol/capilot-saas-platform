...
import { setPreferredInstallPlatform } from "@/store";
...
export function SetupClient({ initialPlatform = "Web", initialSource }: { initialPlatform?: Platform; initialSource?: InstallSource }) {
  ...
  function changePlatform(next: Platform) {
    ...
    setPlatform(next);
    setPreferredInstallPlatform(next);
  ...
  return (
    <div>
      <div className="mb-5 grid grid-cols-3 gap-2 rounded-2xl border border-[var(--border)] bg-white p-2 sm:mb-6">
        {["Choose platform", "Select source", "Install & sign in"].map((label, index) => (
          <div key={label} className={`flex min-h-10 items-center gap-2 rounded-xl px-2.5 text-[9px] font-bold sm:px-3 sm:text-[10px] ${index < 2 ? "bg-[#F0F6F2] text-[#17613F]" : "text-[#7D8680]"}`}>
            <span className={`grid size-5 shrink-0 place-items-center rounded-full ${index < 2 ? "bg-[var(--forest)] text-white" : "bg-[#E8ECE9]"}`}>{index + 1}</span>
      ...
      <aside className="pc-card p-6 sm:p-7">
        ...
        <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-2">
          {installConfig.platforms.map((item) => (
            <button key={item} type="button" onClick={() => changePlatform(item)} className={`flex min-h-12 items-center gap-2 rounded-xl border px-4 text-left text-sm font-semibold transition ${platform === item ? "border-[var(--green)] bg-[var(--green-soft)] text-[#143C2E]" : "border-[var(--border)] bg-white text-[#526058] hover:border-[#ADB9B0]"}`}>
              {platformIcons[item]} {item}
            </button>
          ))}
        </div>
        <div className="mt-6 rounded-2xl bg-[var(--surface-soft)] p-4 text-xs leading-6 text-[var(--muted)]">
      ...
      <section className="pc-card p-6 sm:p-7">
        ...
        <div className="mt-6 grid gap-3">
          {sources.map((item) => (
            <button key={item.id} type="button" onClick={() => setSource(item.id)} className={`flex min-h-20 items-center gap-4 rounded-2xl border p-4 text-left transition ${source === item.id ? "border-[var(--green)] bg-[#F1F7F3] shadow-[0_8px_26px_rgba(30,107,75,.08)]" : "border-[var(--border)] bg-white hover:border-[#AEBAB2]"}`}>
              <span className="grid size-11 shrink-0 place-items-center rounded-xl bg-white text-[var(--green)] shadow-sm">{sourceIcons[item.id]}</span>
              <span className="min-w-0 flex-1">
                <span className="flex flex-wrap items-center gap-2 text-sm font-bold text-[var(--ink)]">{item.label}{item.configured ? <CheckCircle2 className="size-4 text-[var(--success)]" /> : <span className="rounded-full bg-[#F3EFE5] px-2 py-0.5 text-[9px] text-[#8A6418]">Access required</span>}</span>
        ...
        {selected ? (
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <a href={selected.href} target={selected.external ? "_blank" : undefined} rel={selected.external ? "noreferrer" : undefined} onClick={announce} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--forest)] px-5 text-sm font-bold text-white shadow-[0_10px_28px_rgba(20,60,46,.18)] transition hover:-translate-y-px hover:bg-[var(--forest-strong)]">
            ...
            <Link href={`/install?platform=${encodeURIComponent(platform)}`} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-[var(--border-strong)] bg-white px-5 text-sm font-semibold text-[var(--ink-soft)] transition hover:border-[#99A69D] hover:bg-[var(--surface-soft)]">View instructions</Link>
          </div>
        ) : null}
      </section>
      </div>
