    ...
    <form onSubmit={submit} className="mt-7 space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between">
          ...
          <Link href="/contact?intent=account-access" className="text-[11px] font-semibold text-[var(--green)] hover:underline">Need help?</Link>
        </div>
        <div className="relative mt-2">
          ...
          <button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-2 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-lg text-[#6D7771] hover:bg-[var(--surface-soft)]" aria-label={showPassword ? "Hide password" : "Show password"}>
      ...
      <button disabled={loading || demoLoading} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--forest)] px-4 text-sm font-bold text-white shadow-[0_10px_28px_rgba(20,60,46,.14)] transition hover:bg-[var(--forest-strong)] disabled:opacity-60">
      ...
      <button type="button" disabled={demoLoading || loading} onClick={demo} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl border border-[#CAD3CC] bg-white px-4 text-sm font-bold text-[#143C2E] transition hover:bg-[var(--canvas)] disabled:opacity-60">
        {demoLoading ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />}Open demo workspace
      </button>
      <p className="text-center text-xs text-[var(--muted)]">New to PowerChain? <Link href="/get-started" className="font-semibold text-[var(--green)] hover:underline">Create an account</Link></p>
    </form>
  );
}
