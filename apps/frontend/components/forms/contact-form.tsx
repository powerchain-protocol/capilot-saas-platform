  ...
  if (state === "sent") {
    return (
      <div className="pc-card p-8 text-center">
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">Your request is stored in the PowerChain SaaS backend and ready for follow-up.</p>
      </div>
    );
  }
    ...
    <form onSubmit={submit} className="pc-card p-6 sm:p-8">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-xs font-semibold sm:col-span-2">
          How can we help?
          <textarea name="message" required minLength={10} rows={6} className="mt-2 w-full resize-none rounded-xl border border-[var(--border-strong)] p-4 font-normal outline-none focus:border-[var(--green)]" />
        </label>
      </div>
      {error ? <p className="mt-4 rounded-xl bg-[#FFF3F2] px-3 py-2 text-xs text-[#A73535]">{error}</p> : null}
      <button type="submit" disabled={state === "loading"} className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--forest)] text-sm font-bold text-white disabled:opacity-60">
...
function Field({ name, label, type = "text" }: { name: string; label: string; type?: string }) {
  return (
    <label className="text-xs font-semibold">
      {label}
      <input name={name} type={type} required={name !== "company"} className="mt-2 h-12 w-full rounded-xl border border-[var(--border-strong)] px-4 font-normal outline-none focus:border-[var(--green)]" />
    </label>
  );
}
