...
export function Partnership() {
  return (
    <section className="border-t border-[#E3E7E4] bg-white" aria-labelledby="ecosystem-title">
      <div className="pc-shell py-10 sm:py-12">
        ...
        <div className="mt-7 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          {ecosystemItems.map((item, index) => {
            ...
            return <div key={item.name} className="group flex min-h-20 items-center gap-3 rounded-2xl border border-[#E5E9E6] bg-[#FAFBFA] px-4 transition hover:bg-white"><Icon className="size-5 shrink-0 text-[#B0B7B2] transition group-hover:text-[#8C9790]" /><div className="min-w-0"><p className="text-xs font-bold text-[#69736D]">{item.name}</p><p className="mt-1 truncate text-[9px] text-[#A0A7A2]">{item.label}</p></div></div>;
          })}
        </div>
      </div>
    </section>
