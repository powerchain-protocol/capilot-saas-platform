...
export function Downloads() {
  return (
    <section className="bg-[#F7F9F7] py-20 sm:py-24">
      <div className="pc-shell">
        <div className="grid gap-10 overflow-hidden rounded-[28px] border border-[#DCE2DD] bg-white p-6 shadow-[0_24px_80px_rgba(16,21,19,.055)] sm:p-10 lg:grid-cols-[.78fr_1.22fr] lg:p-14">
          <div className="relative">
            ...
            <div className="relative">
              ...
              <p className="mt-5 max-w-xl text-sm leading-7 text-[#66706A]">Use the same identity, assets, operational context and governed workflows across browser, mobile and desktop surfaces.</p>
              <div className="mt-7 grid gap-2 text-xs text-[#5C685F] sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                {["One PowerChain identity", "Synced operational context", "Responsive product surfaces", "Signed distribution channels"].map((item) => <span key={item} className="flex items-center gap-2 rounded-xl bg-[#F7F9F7] px-3 py-2.5"><ShieldCheck className="size-4 shrink-0 text-[#167A4A]" />{item}</span>)}
              </div>
          ...
          <div className="grid content-start gap-3 sm:grid-cols-2">
            ...
            <div className="flex items-start gap-3 rounded-2xl border border-[#D7DED9] bg-[#F8FAF8] px-4 py-3 text-[10px] leading-5 text-[#66706A] sm:col-span-2"><MonitorSmartphone className="mt-0.5 size-4 shrink-0 text-[#167A4A]" /><span>Install sources are configuration-driven. Missing production URLs fail closed and route to a real access-request workflow rather than a dead download.</span></div>
          </div>
        </div>
      </div>
    </section>
