...
function PhoneNav({ active }: { active: string }) {
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 grid grid-cols-5 border-t border-[#E5E9E6] bg-white/96 px-1 py-2 backdrop-blur">
      {items.map(([item, Icon]) => (
        <span key={item} className={`grid place-items-center gap-1 text-center text-[7px] font-semibold ${active === item ? "text-[#167A4A]" : "text-[#7A847E]"}`}>
    ...
    <div className="relative mx-auto flex h-[650px] w-full max-w-[740px] items-end justify-center overflow-visible sm:h-[675px]">
      ...
      <Phone className="absolute bottom-7 left-[0%] hidden -rotate-[1.5deg] scale-[.82] lg:block xl:left-[1%] xl:scale-[.88]">
        <div className="px-4">
          ...
          <div className="mt-1 flex items-center justify-between"><h3 className="text-lg font-bold tracking-[-.03em]">Operations</h3><span className="rounded-full bg-[#EEF7F1] px-2 py-1 text-[7px] font-bold text-[#167A4A]">LIVE</span></div>
          <div className="mt-4 rounded-2xl border border-[#E4E8E5] bg-white p-3 shadow-sm">
            <span className="text-[9px] font-semibold text-[#1E6B4B]">Verified Energy</span>
            <div className="mt-2 flex items-end justify-between"><b className="text-xl">24,812 MWh</b><span className="text-[8px] font-bold text-[#167A4A]">▲ 12.4%</span></div>
          ...
          <div className="mt-3 rounded-xl border border-[#E4E8E5] bg-white p-3"><p className="text-[8px] font-bold">Recent activity</p><p className="mt-2 text-[8px] text-[#69736D]">Wind Farm North · batch verified</p><p className="mt-1 text-[8px] text-[#69736D]">Settlement reconciled · 45m ago</p></div>
        </div>
        ...
        <div className="px-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="grid size-8 place-items-center rounded-xl bg-[#F2F5F3]"><LogoMark className="size-6" priority /></span>
            ...
            <span className="rounded-full border border-[#DCE9E0] bg-[#EEF7F1] px-2 py-1 text-[7px] font-semibold text-[#167A4A]">Onchain-ready</span>
          </div>

          <div className="mt-7">
            <Sparkles className="size-6 text-[#167A4A]"/>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            {["Analyze performance","Review approvals","Summarize energy","Create report"].map((item) => <Link href="/dashboard/copilot" key={item} className="rounded-xl border border-[#E1E6E2] bg-white p-2.5 text-left text-[8px] font-semibold shadow-sm transition hover:border-[#BBC7BE] hover:bg-[#F8FAF8]">{item}</Link>)}
          </div>

          <div className="mt-4 rounded-2xl border border-[#DFE5E0] bg-white p-3 shadow-sm">
            <div className="flex items-start gap-2">
              <div className="mt-1 grid size-6 shrink-0 place-items-center rounded-full bg-[#E7F1EB]"><BarChart3 className="size-3 text-[#167A4A]"/></div>
              <div><p className="text-[9px] font-bold">Solar Farm 45 performance</p><p className="mt-1 text-[8px] leading-4 text-[#657069]">24,812 MWh · 98% availability · no critical anomalies.</p></div>
            </div>
          ...
          <div className="absolute inset-x-4 bottom-14 flex h-11 items-center rounded-full border border-[#D9DEDA] bg-white px-3 shadow-[0_12px_32px_rgba(16,21,19,.12)]">
      ...
      <Phone className="absolute bottom-7 right-[0%] hidden rotate-[1.5deg] scale-[.82] lg:block xl:right-[1%] xl:scale-[.88]">
        <div className="px-4">
          ...
          <div className="mt-3 flex items-center justify-between gap-2"><span className="text-[8px] text-[#647069]">Mojave, California · Solar</span><span className="rounded-full bg-[#EEF7F1] px-2 py-1 text-[7px] font-bold text-[#167A4A]">98% operational</span></div>
          ...
          <div className="mt-4 rounded-2xl border border-[#E1E5E2] bg-white p-3"><div className="flex justify-between"><b className="text-[9px]">Evidence & provenance</b><ShieldCheck className="size-3 text-[#167A4A]"/></div>{["Oracle attestation","Meter readings","Weather context"].map(item => <div key={item} className="mt-3 flex items-center gap-2 text-[8px]"><CheckCircle2 className="size-3 text-[#167A4A]"/><span>{item}</span><span className="ml-auto text-[#167A4A]">Verified</span></div>)}</div>
          <Link href="/dashboard/assets" className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#143C2E] text-[9px] font-bold text-white">Open asset <ChevronRight className="size-3"/></Link>
        </div>
...
function Stat({title,value,icon}:{title:string;value:string;icon:React.ReactNode}) {
  return <div className="rounded-xl border border-[#E3E7E4] bg-white p-2.5"><div className="flex items-center gap-1 text-[#1E6B4B] [&_svg]:size-3"><span>{icon}</span><span className="text-[7px] font-semibold text-[#77817B]">{title}</span></div><b className="mt-1 block text-sm">{value}</b></div>;
}
