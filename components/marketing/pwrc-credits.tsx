  ...
  return (
    <section id="pwrc" className="bg-white py-24">
      <div className="pc-shell">
        <div className="overflow-hidden rounded-[28px] border border-[#D9DEDA] bg-[linear-gradient(140deg,#FBFCFB_0%,#F4F7F5_58%,#EAF2ED_100%)]">
          <div className="grid gap-10 p-7 sm:p-10 lg:grid-cols-[.85fr_1.15fr] lg:p-14">
            <div className="flex flex-col justify-center">
              ...
              <p className="mt-5 max-w-xl text-sm leading-7 text-[#66706A]">
              ...
              <div className="mt-7 flex flex-wrap gap-2">
                {["Available", "Reserved", "Spent", "Receipted"].map((state) => (
                  <span key={state} className="rounded-full border border-[#D7DED9] bg-white px-3 py-1.5 text-[11px] font-semibold text-[#526059]">
                    {state}
                  </span>
                ))}
              </div>
            ...
            <div className="rounded-[24px] border border-white/80 bg-white/88 p-5 shadow-[0_22px_60px_rgba(16,21,19,.07)] backdrop-blur sm:p-6">
              <div className="flex items-center justify-between gap-4 border-b border-[#E8ECE9] pb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="block text-[11px] text-[#7B8580]">Available balance</span>
                    <b className="mt-1 block text-2xl tracking-[-.035em]">256,721 PWRC</b>
                  </div>
                </div>
                <span className="rounded-full bg-[#EDF6F0] px-3 py-1 text-[10px] font-bold text-[#167A4A]">Healthy</span>
              </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-5">
                {lifecycle.map(([title, desc, Icon], index) => (
                  <div key={title} className="relative rounded-2xl border border-[#E2E7E3] bg-[#FBFCFB] p-4 sm:min-h-[150px]">
                    <span className="grid size-9 place-items-center rounded-xl bg-[#EEF5F0] text-[#1E6B4B]"><Icon className="size-4" /></span>
              ...
              <div className="mt-4 flex items-center gap-3 rounded-2xl border border-[#E2E7E3] bg-[#F7F9F7] px-4 py-3 text-[10px] text-[#66706A]">
                <CheckCircle2 className="size-4 shrink-0 text-[#167A4A]" />
