  ...
  return (
    <article className="rounded-2xl border border-[#E2E7E3] bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        {typeof configured === "boolean" ? <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[9px] font-bold ${configured ? "bg-[#EDF7F0] text-[#167A4A]" : "bg-[#F2F4F2] text-[#7B8580]"}`}>{configured ? <CircleCheck className="size-3" /> : <CircleDashed className="size-3" />}{configured ? "Configured" : "Optional"}</span> : null}
      </div>
