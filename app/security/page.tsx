...
export default function Page() {
  return <><Navbar /><main className="bg-white py-24"><div className="pc-shell"><p className="pc-kicker">Security</p><h1 className="mt-4 max-w-4xl text-5xl font-bold tracking-[-.05em]">Governed by default.</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-[#66706A]">PowerChain separates identity, workspace authorization, Copilot analysis, approval, external providers, and execution boundaries. Current-session IP visibility is masked by default and raw IP addresses are not persisted by the reference application database.</p><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{items.map(([title, description, Icon]) => <div key={title} className="pc-card p-6"><Icon className="size-6 text-[#1E6B4B]" /><h2 className="mt-5 font-bold">{title}</h2><p className="mt-2 text-sm leading-6 text-[#66706A]">{description}</p></div>)}</div></div></main><Footer /></>;
}
